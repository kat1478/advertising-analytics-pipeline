...
# Per-campaign cost tier: multipliers applied to base CPM
# This ensures a realistic spread of campaign efficiency
_CAMPAIGN_COST_TIERS = {
    "high": 2.5,    # expensive traffic, tends to produce weak ROAS
    "medium": 1.0,  # baseline
    "low": 0.5,     # cheap traffic, can produce good ROAS if conversions hold
}
...
def generate_campaigns(seed, num_campaigns=20):
    ...
    # ~25% high, ~50% medium, ~25% low → realistic spread
    tier_choices = ['high'] * 5 + ['medium'] * 10 + ['low'] * 5
...
def generate_costs(seed, impressions_df, campaigns_df):
    ...
    Base CPM (cost per 1,000 impressions) is 5–15 PLN, scaled by each
    campaign's cost tier multiplier.  This produces campaign-level cost
    totals in a realistic range relative to the revenues generated by
    conversions, targeting an overall portfolio ROAS of roughly 2–8.
    """
    ...
    tier_multipliers = {'high': 2.5, 'medium': 1.0, 'low': 0.5}

    def _cpm_for_campaign(campaign_id):
        tier = tier_map.get(campaign_id, 'medium')
        return tier_multipliers[tier]

    base_cpm = np.random.uniform(200.0, 600.0, len(daily_impressions))  # PLN per 1000 impressions
    tier_mult = daily_impressions['campaign_id'].map(tier_multipliers).fillna(1.0).values
    # Wait — tier_map returns tier name; we need the numeric multiplier
    tier_mult = daily_impressions['campaign_id'].map(
        lambda cid: tier_multipliers.get(tier_map.get(cid, 'medium'), 1.0)
    ).values
